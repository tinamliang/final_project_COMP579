...
class DQN:
    ...
    def __init__(self,
        ...
        self.history = {'AverageReturn': [], 'MovingAvg': []}
        self.rewards = []
    ...
    def create_network(self, input, trainable):
        if trainable:
            wr = tf.keras.regularizers.l2(0.5 * (self.regularization))
        else:
            wr = None
    ...
    def play_episode(self):
        ...
        while True:
            ...
            for _ in range(self.frame_skip):
                observation, r, done, _, _ = self.env.step(action)
                if self.render:
                    self.env.render()
                reward += r
                if done:
...
class CarRacingDQN(DQN):
    ...
    def process_image(obs):
        return 2 * color.rgb2gray(obs[0]) - 1.0
